...
module.exports = async (client, message) => {
  ...
  if (message.content.match(new RegExp(`^<@!?${client.user.id}>( |)$`))) {
    return message.reply(`Meu prefixo é: \`${prefixesdatabase.prefix}\``);
  }
  ...
  if (timestamps.has(message.author.id + cmd.name)) {
    ...
    if (now < expirationTime) {
      ...
      return message.reply(
        ...
        }${command}**, Por favor espere por**${timeLeft.toFixed(
          1
        )} segundo para cooldowns!**`
      );
    }
  }
  ...
  try {
    ...
    console.log(`${message.author.username} usando comando ${prefixesdatabase.prefix}${command}`);
  }
};
